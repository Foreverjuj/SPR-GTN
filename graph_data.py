...
from graph_embding import load_predicted_PDB
import numpy as np
import os
from graph_embding import exchange_seq
import sys
...
class GoTermDataset(Dataset):
    ...
    def __init__(self, set_type, task, AF2model=False):
        ...
        self.task = task
        if set_type != 'AF2test':
            prot2annot, goterms, gonames, counts = load_GO_annot("data/nrPDB-GO_2024.06.24_annot.tsv")
        else:
        ...
        if AF2model:
            ...
            self.pdbch_list_af = torch.load(os.path.join(self.processed_dir, f"AF2{set_type}_pdbch.pt"))[
                f"{set_type}_pdbch"]
